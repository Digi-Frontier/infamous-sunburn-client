...
async function updateProgressBar(duration, finished) {
  if (finished) {
    ...
    const progressBar = document.getElementById("i");

    progressBar.style.width = "100%";
    document.getElementById("s").textContent = "100%";
    return;
  }

  const progressBar = document.getElementById("i");
  const progressText = document.getElementById("s");
...
function addInputImageEventListener() {
  ...
  if (inputImageElement) {
    inputImageElement.addEventListener("change", (event) => {
      ...
      if (selectedImage) {
        document.querySelector("._").style.display = "none";
...
function displayInputImage(imageFile) {
  const imagesContainer = document.getElementById("t");
...
const modalService = () => {
  ...
  function triggerEvent(button, modal) {
    const modalBody = modal.querySelector(".c");
